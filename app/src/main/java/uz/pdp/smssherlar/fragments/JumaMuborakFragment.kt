...
import kotlinx.android.synthetic.main.fragment_juma_muborak.view.*
...
class JumaMuborakFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        root = inflater.inflate(R.layout.fragment_juma_muborak, container, false)
        val poemDao = AppDatabase.gatDatabase(root.context).poemDao()
        firebaseDatabase = FirebaseDatabase.getInstance()
        databaseReference = firebaseDatabase.getReference("all/juma")
        databaseReference.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                root.linear_back_juma.setOnClickListener {
                    fragmentManager?.beginTransaction()
                        ?.replace(R.id.container, TabrikCategoryFragment())
                        ?.addToBackStack(TabrikCategoryFragment.toString())
                        ?.commit()
                }
                root.juma_muborak_rv.adapter =
                    PoemAdapter(list, object : PoemAdapter.OnClickListener {
                        override fun onItemClickListener(poemModel: PoemModel) {
