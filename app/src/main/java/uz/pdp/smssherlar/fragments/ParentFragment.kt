...
class ParentFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        databaseReference = firebaseDatabase.getReference("all/parent")
        databaseReference.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                root.linear_back_parent.setOnClickListener {
                    fragmentManager?.beginTransaction()
                        ?.replace(R.id.container, HomeFragment())
                        ?.addToBackStack(HomeFragment.toString())
                        ?.commit()
                }
                root.parent_rv.adapter = PoemAdapter(list, object : PoemAdapter.OnClickListener {
