...
import kotlinx.android.synthetic.main.fragment_miss.view.*
...
class BoredomFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        root = inflater.inflate(R.layout.fragment_miss, container, false)
        val poemDao = AppDatabase.gatDatabase(root.context).poemDao()
        firebaseDatabase = FirebaseDatabase.getInstance()
        databaseReference = firebaseDatabase.getReference("all/miss")
        databaseReference.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                root.linear_back_miss.setOnClickListener {
                    fragmentManager?.beginTransaction()
                        ?.replace(R.id.container, HomeFragment())
                        ?.addToBackStack(HomeFragment.toString())
                        ?.commit()
                }
                root.miss_rv.adapter = PoemAdapter(list, object : PoemAdapter.OnClickListener {
