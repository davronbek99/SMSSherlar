...
class JokeFragmentFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        root = inflater.inflate(R.layout.fragment_joke, container, false)
        val poemDao = AppDatabase.gatDatabase(root.context).poemDao()
        firebaseDatabase = FirebaseDatabase.getInstance()
        databaseReference = firebaseDatabase.getReference("all/joke")
        databaseReference.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                root.linear_back_joke.setOnClickListener {
                    fragmentManager?.beginTransaction()
                        ?.replace(R.id.container, HomeFragment())
                        ?.addToBackStack(HomeFragment.toString())
                        ?.commit()
                }
                root.joke_rv.adapter = PoemAdapter(list, object : PoemAdapter.OnClickListener {
